...
public class CompaniesSearchPage extends FluentWebDriverPage {
    ...
    public boolean verifyAdvisorCompanyName(int RowIndex){
        ...
        try{
            ...
            linksCount = linksCount/2;
            for(int i=1;i<=linksCount;i++){
                String strServiceCompanyName = ReadData.readDataExcel("CompaniesFieldDetails", RowIndex, "RealEstateAdvisorCompany");
                String strCompanyName = findElement(By.xpath("//tbody[@id='searchResults']/tr["+i+"]/td/a/strong")).getText();
    ...
    public boolean verifyDealSponsorCompanyName(int RowIndex){
        ...
        try{
            ...
            //linksCount = linksCount/2;
            for(int i=1;i<=linksCount;i++){
                String strDealSponsorCompanyName = ReadData.readDataExcel("CompaniesFieldDetails", RowIndex, "VerifyCompanyName");
                String strCompanyName = findElement(By.xpath("//tbody[@id='searchResults']/tr["+i+"]/td/a/strong")).getText();
    ...
    public boolean verifyDealSponsorCompany(int RowIndex){
        //wait until page gets loaded
        WaitUtil.simpleSleep(5000);
        boolean status = false;
        try{
